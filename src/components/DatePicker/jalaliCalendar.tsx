...
const cmp = (a: Date, b: Date) =>
  clampToDay(a).getTime() - clampToDay(b).getTime();
const sameDay = (a: Date, b: Date) => cmp(a, b) === 0;
const betweenOpen = (d: Date, s?: Date | null, e?: Date | null) =>
  !!(s && e) &&
  clampToDay(s).getTime() < clampToDay(d).getTime() &&
  clampToDay(d).getTime() < clampToDay(e).getTime();
...
const VARIANTS: Record<
  NonNullable<JalaliCalendarProps["variant"]>,
  Required<
    Pick<
      CalendarVars,
      | "selectedBg"
...
> = {
  primary: {
    ...
    rangeBg: "#dbeafe",
    rangeColor: "#1e3a8a",
    hoverBg: "rgba(37,99,235,.08)",
    todayRing: "rgba(37,99,235,.35)",
    focusRing: "#2563eb",
    selectedRadius: "9999px",
    rangeRadius: "9999px",
  },
  secondary: {
    selectedBg: "#6b7280",
    ...
    rangeBg: "#e5e7eb",
    rangeColor: "#111827",
    hoverBg: "rgba(107,114,128,.10)",
    todayRing: "rgba(107,114,128,.35)",
    focusRing: "#6b7280",
    selectedRadius: "9999px",
    rangeRadius: "9999px",
  },
  success: {
    ...
    rangeBg: "#d1fae5",
    rangeColor: "#065f46",
    hoverBg: "rgba(16,185,129,.10)",
    todayRing: "rgba(16,185,129,.35)",
    focusRing: "#10b981",
    selectedRadius: "9999px",
    rangeRadius: "9999px",
  },
  danger: {
    ...
    rangeBg: "#fee2e2",
    rangeColor: "#7f1d1d",
    hoverBg: "rgba(239,68,68,.10)",
    todayRing: "rgba(239,68,68,.35)",
    focusRing: "#ef4444",
    selectedRadius: "9999px",
    rangeRadius: "9999px",
  },
  warning: {
    ...
    rangeBg: "#fef3c7",
    rangeColor: "#78350f",
    hoverBg: "rgba(245,158,11,.10)",
    todayRing: "rgba(245,158,11,.35)",
    focusRing: "#f59e0b",
    selectedRadius: "9999px",
    rangeRadius: "9999px",
  },
  neutral: {
    ...
    rangeBg: "#e2e8f0",
    rangeColor: "#0f172a",
    hoverBg: "rgba(71,85,105,.10)",
    todayRing: "rgba(71,85,105,.35)",
    focusRing: "#475569",
    selectedRadius: "9999px",
    rangeRadius: "9999px",
  },
...
export const JalaliCalendar: React.FC<JalaliCalendarProps> = ({
  ...
  const setView = (m: { jy: number; jm: number }) => {
    onMonthChange?.(m);
  ...
  >(defaultValue);
  const cur =
    (isCtrl ? value : internalValue) ??
    (mode === "single" ? null : { start: null, end: null });
  const setVal = (v: Date | RangeValue | null) => {
    if (!isCtrl) setInternalValue(v);
    onChange?.(v);
  };
  ...
  if (showOutsideDays && shift > 0) {
    ...
    for (let i = prevLen - shift + 1; i <= prevLen; i++)
      cells.push({
        gDate: gFromJ(py, pj, i),
        jy: py,
        jm: pj,
        jd: i,
        outside: true,
  ...
  for (let d = 1; d <= daysInMonth; d++)
    cells.push({ gDate: gFromJ(jy, jm, d), jy, jm, jd: d, outside: false });
  ...
  const range =
    mode === "range" && cur && !(cur instanceof Date)
      ? (cur as RangeValue)
      : { start: null, end: null };
  const { today } = todayJ();
...
const onDayClick = (g: Date) => {
  ...
  const range = mode === "range" && cur && !(cur instanceof Date)
    ? (cur as RangeValue)
    : { start: null, end: null };
  ...
  if (!start || (start && end)) {
    setVal({ start: g, end: null });
  } else {
    if (cmp(g, start) < 0) {
      setVal({ start: g, end: start }); // swap
    } else {
      setVal({ start, end: g });
    }
  }
  ...
  if (vars?.selectedColor)
    (varStyle as any)["--dp-selected-color"] = vars.selectedColor;
  if (vars?.selectedRadius)
    (varStyle as any)["--dp-selected-radius"] =
      typeof vars.selectedRadius === "number"
        ? `${vars.selectedRadius}px`
        : vars.selectedRadius;
  ...
  if (vars?.rangeRadius)
    (varStyle as any)["--dp-range-radius"] =
      typeof vars.rangeRadius === "number"
        ? `${vars.rangeRadius}px`
        : vars.rangeRadius;
  ...
  if (vars?.outsideColor)
    (varStyle as any)["--dp-outside-color"] = vars.outsideColor;
  if (vars?.disabledColor)
    (varStyle as any)["--dp-disabled-color"] = vars.disabledColor;

  // کلاس‌ها
  const rootCn = twMerge(clsx("font-ui select-none", classNames?.root));
  const headerCn = twMerge(
    clsx("flex items-center justify-between mb-3", classNames?.header)
  );
  const navCn = twMerge(
    clsx("inline-flex items-center gap-1", classNames?.nav)
  );
    ...
    >
      ...
      <div className={headerCn}>
        ...
        <div className={titleCn}>
          {toPersianDigits
            ? toFaDigits(`${PERSIAN_MONTHS[jm - 1]} ${jy}`)
            : `${PERSIAN_MONTHS[jm - 1]} ${jy}`}
        </div>
        <div className={navCn}>
          <button
            type="button"
      ...
      <div className={gridCn} role="grid" aria-readonly>
        {cells.map((c, i) => {
          if (c.invalid || !isValidJalaaliDate(c.jy, c.jm, c.jd))
          ...
          const selected =
            ...
            (mode === "range" &&
              range.start &&
              range.end &&
              (sameDay(g, range.start) || sameDay(g, range.end)));
          const between =
            mode === "range" && betweenOpen(g, range.start, range.end);
          const isRangeStart =
            mode === "range" && range.start && sameDay(g, range.start);
          const isRangeEnd =
            mode === "range" && range.end && sameDay(g, range.end);
          ...
          const dayState = clsx(
            ...
            isToday &&
              "ring-1 ring-inset ring-[var(--dp-today-ring,rgba(37,99,235,.35))]",
            selected &&
            ...
            between &&
              "bg-[var(--dp-range-bg,#dbeafe)] text-[var(--dp-range-color,#1e3a8a)]",
            c.outside &&
              showOutsideDays &&
              "text-[var(--dp-outside-color,#9ca3af)]"
          );
            ...
            clsx(
              ...
              isRangeStart &&
                (classNames?.dayRangeStart ??
                  "rounded-l-[var(--dp-range-radius,9999px)]"),
              isRangeEnd &&
                (classNames?.dayRangeEnd ??
                  "rounded-r-[var(--dp-range-radius,9999px)]")
            )
          );

          const label = toPersianDigits
            ? toFaDigits(String(c.jd))
            : String(c.jd);
